...
message_block::msg_entity* message_block::_fork_branch(
    msg_entity const* parent, std::string_view name, bool initial_subscribe_state) {
...
message_block::proxy message_block::fork(const std::string& n) {
  if (auto _lck = std::unique_lock{_sort_merge_lock}) {
    ...
    if (_msg_promise) {
      ...
      std::transform(_table.begin(), _table.end(),
                     ...
                     [](std::pair<const uint64_t, msg_entity> const& g) { return g.second.body; });
...
std::vector<message_block*>& message_block::_all() noexcept {
  static std::vector<message_block*> inst;
  return inst;
}

array_view<message_block*> message_block::all_blocks() noexcept {
  return _all();
}

std::shared_future<message_block::msg_fetch_result> message_block::async_fetch_request() {
...
message_block::proxy::~proxy() noexcept {
  if (!_owner) { return; }
...
message_block::variant_type& message_block::proxy::data() noexcept {
...
void perfkit::sort_messages_by_rule(message_block::fetched_messages& msg) noexcept {
  std::sort(
      msg.begin(), msg.end(),
      [](message_block::msg_type const& a, message_block::msg_type const& b) {
